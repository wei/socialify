...
const Preview: React.FC = () => {
  ...
  const copyMarkdown = () => {
    const screenshotImageUrl = getImageUrl()
    const ogTag = `![${config.name?.state ? `${config.name.value}` : ''}](${screenshotImageUrl})`
  ...
  const copyImageTag = () => {
    const screenshotImageUrl = getImageUrl()
    const ogTag = `<img src="${screenshotImageUrl}" ${config.name?.state ? `alt="${config.name.value}" ` : ''}width="640" height="320" />`
    const success = toClipboard(ogTag)
    if (success) {
      notification.success({
        message: 'Success',
